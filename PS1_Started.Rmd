...
```
### Group Member 1: Mason Putt
### Group Member 2: Alyssa McHoes
### Group Member 3: Jiayi Zhang
...
Estimates of $\beta_1$ and $\beta_2$ are biased because $V_1$ and $V_2$ are highly correlated with one another, creating multicollinearity.
...
$\beta_1$ is biased because it captures the effect of both $V_1$ and $V_2$ on $Y$, creating omitted variable bias.

**Question 3: Generate a new variable $Y_{adj}$ called adjY such that $Y_{adj}=Y-\beta_2*V_2$. Then regress $Y_{adj}$ on $V_1$. Is your estimate of $\beta_1$ biased? Can you explain why/why not?**
...
```
$\beta_1$ is still biased because again it captures the effect of both $V_1$ and $V_2$ on $Y$, creating omitted variable bias.
...
plotted <- ggplot(out, aes(V1, y = value, color = variable)) + 
   ...
   geom_smooth(method='lm', aes(y = Y), col = "black")+
   geom_smooth(method='lm', aes(y = adjY), col = "black") +
   scale_color_manual(name="Variable", values = c("Y"= "steelblue3", "adjY"="red"))+ 
   theme_classic()+
   ggtitle ("Regression of V1 on Y and Adjusted Y")+ 
   ylab("Value")
